...
fn test_filter_parse_cpm_eq_date() {
    ...
    assert!(
        dict! {"date" => Value::make_date(Date::from_ymd(2020, 5, 18).expect("Invalid date"))}
            .filter(filter)
    );
}
...
fn test_filter_parse_gt_date() {
    let filter = &Filter::try_from("date > 2020-05-18").unwrap();
    assert!(
        dict! {"date" => Value::make_date(Date::from_ymd(2020, 6, 18).expect("Invalid Date"))}
            .filter(filter)
    );
}
...
fn test_filter_parse_path() {
    ...
    assert!(dict! {"dict" => Value::make_dict(dict! {"val" => Value::make_int(1)})}.filter(filter));
}
